...
export default function Auth({ apiBase, onAuthed, onBack }) {
  ...
  async function submit() {
    try {
      setLoading(true);
      const url = mode === "login" ? `${apiBase}/api/auth/login` : `${apiBase}/api/auth/register`;
    ...
    <div className="flex-center" style={{ minHeight: '70vh' }}>
      <div className="ai-card" style={{ maxWidth: '450px', width: '100%' }}>
        <div className="iv-header">
          <h2 className="iv-title">{mode === "login" ? "Welcome Back" : "Create Account"}</h2>
          {onBack && (
            <button className="ai-btn ai-btn-danger" style={{ padding: '5px 12px' }} onClick={onBack}>
              ✖
            </button>
          )}
        </div>
        ...
        {/* Tab Switcher */}
        <div className="ai-nav" style={{ justifyContent: 'center', marginBottom: '25px', background: '#f1f5f9', padding: '5px', borderRadius: '50px' }}>
          <button
            className={`ai-btn ${mode === "login" ? "ai-btn-primary" : ""}`}
            style={{ flex: 1, background: mode !== "login" ? 'transparent' : '', color: mode !== "login" ? '#64748b' : '' }}
          ...
          <button
            ...
            style={{ flex: 1, background: mode !== "register" ? 'transparent' : '', color: mode !== "register" ? '#64748b' : '' }}
        ...
        <div className="ai-muted mt-2" style={{ fontSize: '12px' }}>
          🔐 Secure login enabled. Protected routes like TTS, PDF Upload, and Evaluation require authentication.
        </div>
      </div>
    </div>
  );
